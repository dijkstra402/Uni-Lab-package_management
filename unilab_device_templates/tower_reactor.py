...
class TowerReactor:
    ...
    def reflux(self, mode: int = 0, reflux_ratio: int = 0) -> Dict[str, Any]:
        """
        回流。

        Args:
            mode[运行模式设置]: 运行模式设置。
            reflux_ratio[回流比设置]: 回流比设置。
        """
        pass
    ...
    @property
    @topic_config()
    def current_liquid_level(self) -> int:
        """实际液位高度。"""
        return self.data.get("current_liquid_level", 0)
