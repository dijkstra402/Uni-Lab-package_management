"""
真空离心浓缩仪 — PLC(OPC UA) 交互驱动 (自动生成，请勿手改)

由 generate_plc_drivers.py 依据 device_templates_actions.csv 生成。
继承 OpcUaClientWithSubscription，把「真空离心浓缩仪」标准动作/属性映射为 PLC 节点读写：
...
class VacuumCentrifugalConcentratorPLC(OpcUaClientWithSubscription):
    ...
    def concentrate(self) -> Dict[str, Any]:
        """浓缩：写 Concentrate_Trigger 触发，等待 Concentrate_Complete 完成后复位。"""
        logger.info("浓缩...")
        self.set_node_value("Concentrate_Trigger", True)
        if not self._wait_until_true("Concentrate_Complete", description="浓缩完成"):
            raise ValueError("浓缩失败：动作未完成")
        self.set_node_value("Concentrate_Trigger", False)
        if not self._wait_until_false("Concentrate_Complete", description="浓缩完成复位"):
            raise ValueError("浓缩失败：完成状态复位超时")
        return {"success": True, "message": "浓缩完成"}

    @action(description="真空")
    def vacuum(self) -> Dict[str, Any]:
        """真空：写 Vacuum_Trigger 触发，等待 Vacuum_Complete 完成后复位。"""
        logger.info("真空...")
        self.set_node_value("Vacuum_Trigger", True)
        if not self._wait_until_true("Vacuum_Complete", description="真空完成"):
            raise ValueError("真空失败：动作未完成")
        self.set_node_value("Vacuum_Trigger", False)
        if not self._wait_until_false("Vacuum_Complete", description="真空完成复位"):
            raise ValueError("真空失败：完成状态复位超时")
        return {"success": True, "message": "真空完成"}
    ...
    def concentration_temperature(self) -> float:
        """浓缩温度（读节点 Concentration_Temperature）。"""
        v = self.get_node_value("Concentration_Temperature")
        return v if v is not None else 0.0

    @property
    @topic_config()
    def concentration_time(self) -> float:
        """浓缩时间（读节点 Concentration_Time）。"""
        v = self.get_node_value("Concentration_Time")
        return v if v is not None else 0.0

    @property
    @topic_config()
    def heating_power(self) -> float:
        """加热功率（读节点 Heating_Power）。"""
        v = self.get_node_value("Heating_Power")
        return v if v is not None else 0.0
