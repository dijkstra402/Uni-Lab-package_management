"""
高低温恒温槽 — PLC(OPC UA) 交互驱动 (自动生成，请勿手改)

由 generate_plc_drivers.py 依据 device_templates_actions.csv 生成。
继承 OpcUaClientWithSubscription，把「高低温恒温槽」标准动作/属性映射为 PLC 节点读写：
...
@device(
    id="high_low_temp_thermostatic_bath_plc",
    category=["加热、制冷及空气净化与调节设备", "恒温槽", "高低温恒温槽"],
    description="高低温恒温槽 PLC(OPC UA) 交互驱动：动作映射为节点读写。",
    displayname="高低温恒温槽(PLC)",
)
class HighLowTempThermostaticBathPLC(OpcUaClientWithSubscription):
    ...
    @property
    @topic_config()
    def cooling_system_state(self) -> int:
        """制冷系统状态（读节点 Cooling_System_State）。"""
