"""
固相萃取设备 — PLC(OPC UA) 交互驱动 (自动生成，请勿手改)

由 generate_plc_drivers.py 依据 device_templates_actions.csv 生成。
继承 OpcUaClientWithSubscription，把「固相萃取设备」标准动作/属性映射为 PLC 节点读写：
...
class SolidPhaseExtractorPLC(OpcUaClientWithSubscription):
    ...
    def set_extraction_column_pressure(self, extraction_column_pressure: float = 0.0) -> Dict[str, Any]:
        """设置萃取柱压力：写入设定值节点 Extraction_Column_Pressure_Setpoint。"""
        self.set_node_value("Extraction_Column_Pressure_Setpoint", extraction_column_pressure)
        return {"success": True, "message": "设置萃取柱压力已下发", "extraction_column_pressure": extraction_column_pressure}

    @action(description="设置洗脱液流量")
    def set_eluent_flow(self, eluent_flow: float = 0.0) -> Dict[str, Any]:
        """设置洗脱液流量：写入设定值节点 Eluent_Flow_Setpoint。"""
        self.set_node_value("Eluent_Flow_Setpoint", eluent_flow)
        return {"success": True, "message": "设置洗脱液流量已下发", "eluent_flow": eluent_flow}
    ...
    def sample_loading_completed(self) -> bool:
        """样品加载完成（读节点 Sample_Loading_Completed）。"""
        v = self.get_node_value("Sample_Loading_Completed")
        return v if v is not None else False

    @property
    @topic_config()
    def elution_completed(self) -> bool:
        """洗脱完成（读节点 Elution_Completed）。"""
        v = self.get_node_value("Elution_Completed")
        return v if v is not None else False

    @property
    @topic_config()
    def column_pressure_protection_state(self) -> bool:
        """柱压保护状态（读节点 Column_Pressure_Protection_State）。"""
        v = self.get_node_value("Column_Pressure_Protection_State")
        return v if v is not None else False
    ...
    def current_extraction_column_pressure(self) -> float:
        """实际萃取柱压力（读节点 Current_Extraction_Column_Pressure）。"""
        v = self.get_node_value("Current_Extraction_Column_Pressure")
        return v if v is not None else 0.0

    @property
    @topic_config()
    def current_eluent_flow(self) -> float:
        """实际洗脱液流量（读节点 Current_Eluent_Flow）。"""
        v = self.get_node_value("Current_Eluent_Flow")
        return v if v is not None else 0.0
