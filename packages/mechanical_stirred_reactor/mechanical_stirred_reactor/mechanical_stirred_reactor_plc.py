"""
机械搅拌反应釜 — PLC(OPC UA) 交互驱动 (自动生成，请勿手改)

由 generate_plc_drivers.py 依据 device_templates_actions.csv 生成。
继承 OpcUaClientWithSubscription，把「机械搅拌反应釜」标准动作/属性映射为 PLC 节点读写：
...
class MechanicalStirredReactorPLC(OpcUaClientWithSubscription):
    ...
    def set_frequency(self, frequency: float = 0.0) -> Dict[str, Any]:
        """设置频率：写入设定值节点 Frequency_Setpoint。"""
        self.set_node_value("Frequency_Setpoint", frequency)
        return {"success": True, "message": "设置频率已下发", "frequency": frequency}

    @action(description="变频")
    def set_frequency_2(self) -> Dict[str, Any]:
        """变频：写 Set_Frequency_2_Trigger 触发，等待 Set_Frequency_2_Complete 完成后复位。"""
        logger.info("变频...")
        self.set_node_value("Set_Frequency_2_Trigger", True)
        if not self._wait_until_true("Set_Frequency_2_Complete", description="变频完成"):
            raise ValueError("变频失败：动作未完成")
        self.set_node_value("Set_Frequency_2_Trigger", False)
        if not self._wait_until_false("Set_Frequency_2_Complete", description="变频完成复位"):
            raise ValueError("变频失败：完成状态复位超时")
        return {"success": True, "message": "变频完成"}
    ...
    def current_frequency(self) -> float:
        """实际频率显示（读节点 Current_Frequency）。"""
        v = self.get_node_value("Current_Frequency")
        return v if v is not None else 0.0

    @property
    @topic_config()
    def output_current(self) -> float:
        """输出电流显示（读节点 Output_Current）。"""
        v = self.get_node_value("Output_Current")
        return v if v is not None else 0.0

    @property
    @topic_config()
    def torque_percent(self) -> float:
        """扭矩百分比显示（读节点 Torque_Percent）。"""
        v = self.get_node_value("Torque_Percent")
        return v if v is not None else 0.0
