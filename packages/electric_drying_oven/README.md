...
定义「电热干燥箱」这一设备**大类**的标准动作(`@action`)与状态属性(`@property`)。
...
```
electric_drying_oven/
├── pyproject.toml
├── requirements.txt
├── .github/workflows/check_registry.yml
└── electric_drying_oven/
    ├── __init__.py
    └── electric_drying_oven.py        # 电热干燥箱 标准类定义
```

## 本地验证

```bash
unilab --check_mode --devices ./electric_drying_oven --external_devices_only
```
