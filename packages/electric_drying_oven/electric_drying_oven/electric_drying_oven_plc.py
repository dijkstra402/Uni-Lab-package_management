"""
电热干燥箱 — PLC(OPC UA) 交互驱动 (自动生成，请勿手改)

由 generate_plc_drivers.py 依据 device_templates_actions.csv 生成。
继承 OpcUaClientWithSubscription，把「电热干燥箱」标准动作/属性映射为 PLC 节点读写：
...
@device(
    id="electric_drying_oven_plc",
    category=["电热干燥箱"],
    description="电热干燥箱 PLC(OPC UA) 交互驱动：动作映射为节点读写。",
    display_name="电热干燥箱(PLC)",
)
class ElectricDryingOvenPLC(OpcUaClientWithSubscription):
    ...
    def set_fan_speed(self, fan_speed: float = 0.0) -> Dict[str, Any]:
        """设置风机转速：写入设定值节点 Fan_Speed_Setpoint。"""
        self.set_node_value("Fan_Speed_Setpoint", fan_speed)
        return {"success": True, "message": "设置风机转速已下发", "fan_speed": fan_speed}

    @action(description="设置风门开度")
    def set_damper_opening(self, damper_opening: int = 0) -> Dict[str, Any]:
        """设置风门开度：写入设定值节点 Damper_Opening_Setpoint。"""
        self.set_node_value("Damper_Opening_Setpoint", damper_opening)
        return {"success": True, "message": "设置风门开度已下发", "damper_opening": damper_opening}
    ...
    def current_fan_speed(self) -> float:
        """实际风机转速（读节点 Current_Fan_Speed）。"""
        v = self.get_node_value("Current_Fan_Speed")
        return v if v is not None else 0.0

    @property
    @topic_config()
    def current_damper_opening(self) -> int:
        """实际风门开度（读节点 Current_Damper_Opening）。"""
        v = self.get_node_value("Current_Damper_Opening")
        return v if v is not None else 0
