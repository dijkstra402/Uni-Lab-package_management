"""
化学消毒机 — PLC(OPC UA) 交互驱动 (自动生成，请勿手改)

由 generate_plc_drivers.py 依据 device_templates_actions.csv 生成。
继承 OpcUaClientWithSubscription，把「化学消毒机」标准动作/属性映射为 PLC 节点读写：
...
class ChemicalSterilizerPLC(OpcUaClientWithSubscription):
    ...
    def set_disinfectant_concentration(self, disinfectant_concentration: float = 0.0) -> Dict[str, Any]:
        """设置消毒剂浓度：写入设定值节点 Disinfectant_Concentration_Setpoint。"""
        self.set_node_value("Disinfectant_Concentration_Setpoint", disinfectant_concentration)
        return {"success": True, "message": "设置消毒剂浓度已下发", "disinfectant_concentration": disinfectant_concentration}
    ...
    def stir_state(self) -> bool:
        """搅拌状态（读节点 Stir_State）。"""
        v = self.get_node_value("Stir_State")
        return v if v is not None else False

    @property
    @topic_config()
    def neutralization_wash_state(self) -> bool:
        """中和清洗状态（读节点 Neutralization_Wash_State）。"""
        v = self.get_node_value("Neutralization_Wash_State")
        return v if v is not None else False
    ...
    def disinfectant_concentration_feedback(self) -> float:
        """消毒剂浓度反馈（读节点 Disinfectant_Concentration_Feedback）。"""
        v = self.get_node_value("Disinfectant_Concentration_Feedback")
        return v if v is not None else 0.0
