"""
黑体辐射源 — PLC(OPC UA) 交互驱动 (自动生成，请勿手改)

由 generate_plc_drivers.py 依据 device_templates_actions.csv 生成。
继承 OpcUaClientWithSubscription，把「黑体辐射源」标准动作/属性映射为 PLC 节点读写：
...
class BlackbodyRadiationSourcePLC(OpcUaClientWithSubscription):
    ...
    def set_emissivity(self, emissivity: float = 0.0) -> Dict[str, Any]:
        """设置辐射率：写入设定值节点 Emissivity_Setpoint。"""
        self.set_node_value("Emissivity_Setpoint", emissivity)
        return {"success": True, "message": "设置辐射率已下发", "emissivity": emissivity}

    @action(description="设置辐射强度")
    def set_radiation_intensity(self, radiation_intensity: float = 0.0) -> Dict[str, Any]:
        """设置辐射强度：写入设定值节点 Radiation_Intensity_Setpoint。"""
        self.set_node_value("Radiation_Intensity_Setpoint", radiation_intensity)
        return {"success": True, "message": "设置辐射强度已下发", "radiation_intensity": radiation_intensity}
    ...
    def radiation_output(self) -> Dict[str, Any]:
        """辐射输出：写 Radiation_Output_Trigger 触发，等待 Radiation_Output_Complete 完成后复位。"""
        logger.info("辐射输出...")
        self.set_node_value("Radiation_Output_Trigger", True)
        if not self._wait_until_true("Radiation_Output_Complete", description="辐射输出完成"):
            raise ValueError("辐射输出失败：动作未完成")
        self.set_node_value("Radiation_Output_Trigger", False)
        if not self._wait_until_false("Radiation_Output_Complete", description="辐射输出完成复位"):
            raise ValueError("辐射输出失败：完成状态复位超时")
        return {"success": True, "message": "辐射输出完成"}

    @action(description="辐射率校准")
    def calibrate_emissivity(self) -> Dict[str, Any]:
        """辐射率校准：写 Calibrate_Emissivity_Trigger 触发，等待 Calibrate_Emissivity_Complete 完成后复位。"""
        logger.info("辐射率校准...")
        self.set_node_value("Calibrate_Emissivity_Trigger", True)
        if not self._wait_until_true("Calibrate_Emissivity_Complete", description="辐射率校准完成"):
            raise ValueError("辐射率校准失败：动作未完成")
        self.set_node_value("Calibrate_Emissivity_Trigger", False)
        if not self._wait_until_false("Calibrate_Emissivity_Complete", description="辐射率校准完成复位"):
            raise ValueError("辐射率校准失败：完成状态复位超时")
        return {"success": True, "message": "辐射率校准完成"}
    ...
    def current_emissivity(self) -> int:
        """实际辐射率（读节点 Current_Emissivity）。"""
        v = self.get_node_value("Current_Emissivity")
        return v if v is not None else 0

    @property
    @topic_config()
    def radiation_intensity_feedback(self) -> int:
        """辐射强度反馈（读节点 Radiation_Intensity_Feedback）。"""
        v = self.get_node_value("Radiation_Intensity_Feedback")
        return v if v is not None else 0
    ...
    def radiation_stable_time(self) -> float:
        """辐射稳定时间（读节点 Radiation_Stable_Time）。"""
        v = self.get_node_value("Radiation_Stable_Time")
        return v if v is not None else 0.0

    @property
    @topic_config()
    def preheat_stable_time(self) -> float:
        """预热稳定时间（读节点 Preheat_Stable_Time）。"""
        v = self.get_node_value("Preheat_Stable_Time")
        return v if v is not None else 0.0
