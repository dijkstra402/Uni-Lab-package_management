"""
血液柜 — PLC(OPC UA) 交互驱动 (自动生成，请勿手改)

由 generate_plc_drivers.py 依据 device_templates_actions.csv 生成。
继承 OpcUaClientWithSubscription，把「血液柜」标准动作/属性映射为 PLC 节点读写：
...
@device(
    id="blood_bank_refrigerator_plc",
    category=["加热、制冷及空气净化与调节设备", "冰箱及类似设备", "血液柜"],
    description="血液柜 PLC(OPC UA) 交互驱动：动作映射为节点读写。",
    displayname="血液柜(PLC)",
)
class BloodBankRefrigeratorPLC(OpcUaClientWithSubscription):
    ...
    def set_alarm_temperature(self, alarm_temperature: float = 0.0) -> Dict[str, Any]:
        """设置报警温度：写入设定值节点 Alarm_Temperature_Setpoint。"""
        self.set_node_value("Alarm_Temperature_Setpoint", alarm_temperature)
        return {"success": True, "message": "设置报警温度已下发", "alarm_temperature": alarm_temperature}
    ...
    def backup(self) -> Dict[str, Any]:
        """备份：写 Backup_Trigger 触发，等待 Backup_Complete 完成后复位。"""
        logger.info("备份...")
        self.set_node_value("Backup_Trigger", True)
        if not self._wait_until_true("Backup_Complete", description="备份完成"):
            raise ValueError("备份失败：动作未完成")
        self.set_node_value("Backup_Trigger", False)
        if not self._wait_until_false("Backup_Complete", description="备份完成复位"):
            raise ValueError("备份失败：完成状态复位超时")
    ...
    def data_logging_state(self) -> bool:
        """数据记录状态（读节点 Data_Logging_State）。"""
        v = self.get_node_value("Data_Logging_State")
        return v if v is not None else False
    ...
    def temperature_logging_interval(self) -> float:
        """温度记录间隔（读节点 Temperature_Logging_Interval）。"""
        v = self.get_node_value("Temperature_Logging_Interval")
        return v if v is not None else 0.0
