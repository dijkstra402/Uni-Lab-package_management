"""
激光清洗机 — PLC(OPC UA) 交互驱动 (自动生成，请勿手改)

由 generate_plc_drivers.py 依据 device_templates_actions.csv 生成。
继承 OpcUaClientWithSubscription，把「激光清洗机」标准动作/属性映射为 PLC 节点读写：
...
@device(
    id="laser_cleaner_plc",
    category=["激光清洗机"],
    description="激光清洗机 PLC(OPC UA) 交互驱动：动作映射为节点读写。",
    display_name="激光清洗机(PLC)",
)
class LaserCleanerPLC(OpcUaClientWithSubscription):
    ...
    def set_laser_power(self, laser_power: float = 0.0) -> Dict[str, Any]:
        """设置激光功率：写入设定值节点 Laser_Power_Setpoint。"""
        self.set_node_value("Laser_Power_Setpoint", laser_power)
        return {"success": True, "message": "设置激光功率已下发", "laser_power": laser_power}

    @action(description="设置扫描速度")
    def set_scan_speed(self, scan_speed: float = 0.0) -> Dict[str, Any]:
        """设置扫描速度：写入设定值节点 Scan_Speed_Setpoint。"""
        self.set_node_value("Scan_Speed_Setpoint", scan_speed)
        return {"success": True, "message": "设置扫描速度已下发", "scan_speed": scan_speed}

    @action(description="设置聚焦距离")
    def set_focus_distance(self, focus_distance: float = 0.0) -> Dict[str, Any]:
        """设置聚焦距离：写入设定值节点 Focus_Distance_Setpoint。"""
        self.set_node_value("Focus_Distance_Setpoint", focus_distance)
        return {"success": True, "message": "设置聚焦距离已下发", "focus_distance": focus_distance}
    ...
    def laser_head_moving_state(self) -> bool:
        """激光头移动状态（读节点 Laser_Head_Moving_State）。"""
        v = self.get_node_value("Laser_Head_Moving_State")
        return v if v is not None else False
    ...
    def laser_power_feedback(self) -> float:
        """激光功率反馈（读节点 Laser_Power_Feedback）。"""
        v = self.get_node_value("Laser_Power_Feedback")
        return v if v is not None else 0.0

    @property
    @topic_config()
    def scan_speed_feedback(self) -> float:
        """扫描速度反馈（读节点 Scan_Speed_Feedback）。"""
        v = self.get_node_value("Scan_Speed_Feedback")
        return v if v is not None else 0.0

    @property
    @topic_config()
    def focus_distance_feedback(self) -> float:
        """聚焦距离反馈（读节点 Focus_Distance_Feedback）。"""
        v = self.get_node_value("Focus_Distance_Feedback")
        return v if v is not None else 0.0
