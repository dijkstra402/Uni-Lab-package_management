"""
过滤器 — PLC(OPC UA) 交互驱动 (自动生成，请勿手改)

由 generate_plc_drivers.py 依据 device_templates_actions.csv 生成。
继承 OpcUaClientWithSubscription，把「过滤器」标准动作/属性映射为 PLC 节点读写：
...
class FilterPLC(OpcUaClientWithSubscription):
    ...
    def filter(self) -> Dict[str, Any]:
        """过滤：写 Filter_Trigger 触发，等待 Filter_Complete 完成后复位。"""
        logger.info("过滤...")
        self.set_node_value("Filter_Trigger", True)
        if not self._wait_until_true("Filter_Complete", description="过滤完成"):
            raise ValueError("过滤失败：动作未完成")
        self.set_node_value("Filter_Trigger", False)
        if not self._wait_until_false("Filter_Complete", description="过滤完成复位"):
            raise ValueError("过滤失败：完成状态复位超时")
        return {"success": True, "message": "过滤完成"}

    @action(description="反冲洗")
    def backwash(self) -> Dict[str, Any]:
        """反冲洗：写 Backwash_Trigger 触发，等待 Backwash_Complete 完成后复位。"""
        logger.info("反冲洗...")
        self.set_node_value("Backwash_Trigger", True)
        if not self._wait_until_true("Backwash_Complete", description="反冲洗完成"):
            raise ValueError("反冲洗失败：动作未完成")
        self.set_node_value("Backwash_Trigger", False)
        if not self._wait_until_false("Backwash_Complete", description="反冲洗完成复位"):
            raise ValueError("反冲洗失败：完成状态复位超时")
        return {"success": True, "message": "反冲洗完成"}
    ...
    def filtration_time(self) -> float:
        """过滤时间（读节点 Filtration_Time）。"""
        v = self.get_node_value("Filtration_Time")
        return v if v is not None else 0.0
