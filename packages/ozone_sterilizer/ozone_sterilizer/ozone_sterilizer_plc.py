"""
臭氧消毒机 — PLC(OPC UA) 交互驱动 (自动生成，请勿手改)

由 generate_plc_drivers.py 依据 device_templates_actions.csv 生成。
继承 OpcUaClientWithSubscription，把「臭氧消毒机」标准动作/属性映射为 PLC 节点读写：
...
@device(
    id="ozone_sterilizer_plc",
    category=["臭氧消毒机"],
    description="臭氧消毒机 PLC(OPC UA) 交互驱动：动作映射为节点读写。",
    display_name="臭氧消毒机(PLC)",
)
class OzoneSterilizerPLC(OpcUaClientWithSubscription):
    ...
    def set_ozone_concentration(self, ozone_concentration: float = 0.0) -> Dict[str, Any]:
        """设置臭氧浓度：写入设定值节点 Ozone_Concentration_Setpoint。"""
        self.set_node_value("Ozone_Concentration_Setpoint", ozone_concentration)
        return {"success": True, "message": "设置臭氧浓度已下发", "ozone_concentration": ozone_concentration}
    ...
    def ozone_generator_state(self) -> bool:
        """臭氧发生器状态（读节点 Ozone_Generator_State）。"""
        v = self.get_node_value("Ozone_Generator_State")
        return v if v is not None else False

    @property
    @topic_config()
    def exhaust_treatment_state(self) -> bool:
        """尾气处理状态（读节点 Exhaust_Treatment_State）。"""
        v = self.get_node_value("Exhaust_Treatment_State")
        return v if v is not None else False

    @property
    @topic_config()
    def ventilation_system_state(self) -> bool:
        """通风系统状态（读节点 Ventilation_System_State）。"""
        v = self.get_node_value("Ventilation_System_State")
        return v if v is not None else False
    ...
    def ozone_concentration_feedback(self) -> float:
        """臭氧浓度反馈（读节点 Ozone_Concentration_Feedback）。"""
        v = self.get_node_value("Ozone_Concentration_Feedback")
        return v if v is not None else 0.0
