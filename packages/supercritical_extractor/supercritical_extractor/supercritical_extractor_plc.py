"""
超临界萃取设备 — PLC(OPC UA) 交互驱动 (自动生成，请勿手改)

由 generate_plc_drivers.py 依据 device_templates_actions.csv 生成。
继承 OpcUaClientWithSubscription，把「超临界萃取设备」标准动作/属性映射为 PLC 节点读写：
...
@device(
    id="supercritical_extractor_plc",
    category=["样品处理仪器与设备", "提取设备", "超临界萃取设备"],
    description="超临界萃取设备 PLC(OPC UA) 交互驱动：动作映射为节点读写。",
    displayname="超临界萃取设备(PLC)",
)
class SupercriticalExtractorPLC(OpcUaClientWithSubscription):
    ...
    def set_extraction_pressure(self, extraction_pressure: float = 0.0) -> Dict[str, Any]:
        """设置萃取压力：写入设定值节点 Extraction_Pressure_Setpoint。"""
        self.set_node_value("Extraction_Pressure_Setpoint", extraction_pressure)
        return {"success": True, "message": "设置萃取压力已下发", "extraction_pressure": extraction_pressure}
    ...
    def set_co2_flow(self, co2_flow: float = 0.0) -> Dict[str, Any]:
        """设置CO2流量：写入设定值节点 Co2_Flow_Setpoint。"""
        self.set_node_value("Co2_Flow_Setpoint", co2_flow)
        return {"success": True, "message": "设置CO2流量已下发", "co2_flow": co2_flow}

    @action(description="设置分离压力")
    def set_separation_pressure(self, separation_pressure: float = 0.0) -> Dict[str, Any]:
        """设置分离压力：写入设定值节点 Separation_Pressure_Setpoint。"""
        self.set_node_value("Separation_Pressure_Setpoint", separation_pressure)
        return {"success": True, "message": "设置分离压力已下发", "separation_pressure": separation_pressure}
    ...
    def current_extraction_pressure(self) -> float:
        """实际萃取压力（读节点 Current_Extraction_Pressure）。"""
        v = self.get_node_value("Current_Extraction_Pressure")
        return v if v is not None else 0.0
    ...
    def current_co2_flow(self) -> float:
        """实际CO2流量（读节点 Current_Co2_Flow）。"""
        v = self.get_node_value("Current_Co2_Flow")
        return v if v is not None else 0.0

    @property
    @topic_config()
    def current_separation_pressure(self) -> float:
        """实际分离压力（读节点 Current_Separation_Pressure）。"""
        v = self.get_node_value("Current_Separation_Pressure")
        return v if v is not None else 0.0
