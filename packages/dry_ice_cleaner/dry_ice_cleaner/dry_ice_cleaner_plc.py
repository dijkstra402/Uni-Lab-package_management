"""
干冰清洗机 — PLC(OPC UA) 交互驱动 (自动生成，请勿手改)

由 generate_plc_drivers.py 依据 device_templates_actions.csv 生成。
继承 OpcUaClientWithSubscription，把「干冰清洗机」标准动作/属性映射为 PLC 节点读写：
...
@device(
    id="dry_ice_cleaner_plc",
    category=["干冰清洗机"],
    description="干冰清洗机 PLC(OPC UA) 交互驱动：动作映射为节点读写。",
    display_name="干冰清洗机(PLC)",
)
class DryIceCleanerPLC(OpcUaClientWithSubscription):
    ...
    def set_dry_ice_amount(self, dry_ice_amount: float = 0.0) -> Dict[str, Any]:
        """设置干冰用量：写入设定值节点 Dry_Ice_Amount_Setpoint。"""
        self.set_node_value("Dry_Ice_Amount_Setpoint", dry_ice_amount)
        return {"success": True, "message": "设置干冰用量已下发", "dry_ice_amount": dry_ice_amount}

    @action(description="设置喷射压力")
    def set_jet_pressure(self, jet_pressure: float = 0.0) -> Dict[str, Any]:
        """设置喷射压力：写入设定值节点 Jet_Pressure_Setpoint。"""
        self.set_node_value("Jet_Pressure_Setpoint", jet_pressure)
        return {"success": True, "message": "设置喷射压力已下发", "jet_pressure": jet_pressure}

    @action(description="设置喷嘴距离")
    def set_nozzle_distance(self, nozzle_distance: float = 0.0) -> Dict[str, Any]:
        """设置喷嘴距离：写入设定值节点 Nozzle_Distance_Setpoint。"""
        self.set_node_value("Nozzle_Distance_Setpoint", nozzle_distance)
        return {"success": True, "message": "设置喷嘴距离已下发", "nozzle_distance": nozzle_distance}
    ...
    def compressed_air_state(self) -> bool:
        """压缩空气状态（读节点 Compressed_Air_State）。"""
        v = self.get_node_value("Compressed_Air_State")
        return v if v is not None else False
    ...
    def dry_ice_amount_feedback(self) -> str:
        """干冰用量反馈（读节点 Dry_Ice_Amount_Feedback）。"""
        v = self.get_node_value("Dry_Ice_Amount_Feedback")
        return v if v is not None else ""

    @property
    @topic_config()
    def jet_pressure_feedback(self) -> str:
        """喷射压力反馈（读节点 Jet_Pressure_Feedback）。"""
        v = self.get_node_value("Jet_Pressure_Feedback")
        return v if v is not None else ""

    @property
    @topic_config()
    def nozzle_distance_feedback(self) -> str:
        """喷嘴距离反馈（读节点 Nozzle_Distance_Feedback）。"""
        v = self.get_node_value("Nozzle_Distance_Feedback")
        return v if v is not None else ""
