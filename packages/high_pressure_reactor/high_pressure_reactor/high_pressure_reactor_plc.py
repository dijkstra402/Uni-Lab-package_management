"""
高压反应器 — PLC(OPC UA) 交互驱动 (自动生成，请勿手改)

由 generate_plc_drivers.py 依据 device_templates_actions.csv 生成。
继承 OpcUaClientWithSubscription，把「高压反应器」标准动作/属性映射为 PLC 节点读写：
...
class HighPressureReactorPLC(OpcUaClientWithSubscription):
    ...
    def set_pressurization(self, pressurization: float = 0.0) -> Dict[str, Any]:
        """设置充压：写入设定值节点 Pressurization_Setpoint。"""
        self.set_node_value("Pressurization_Setpoint", pressurization)
        return {"success": True, "message": "设置充压已下发", "pressurization": pressurization}
