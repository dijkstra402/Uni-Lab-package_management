"""
管式炉 — PLC(OPC UA) 交互驱动 (自动生成，请勿手改)

由 generate_plc_drivers.py 依据 device_templates_actions.csv 生成。
继承 OpcUaClientWithSubscription，把「管式炉」标准动作/属性映射为 PLC 节点读写：
...
@device(
    id="tube_furnace_plc",
    category=["管式炉"],
    description="管式炉 PLC(OPC UA) 交互驱动：动作映射为节点读写。",
    display_name="管式炉(PLC)",
)
class TubeFurnacePLC(OpcUaClientWithSubscription):
    ...
    def set_furnace_tube_speed(self, furnace_tube_speed: float = 0.0) -> Dict[str, Any]:
        """设置炉管转速：写入设定值节点 Furnace_Tube_Speed_Setpoint。"""
        self.set_node_value("Furnace_Tube_Speed_Setpoint", furnace_tube_speed)
        return {"success": True, "message": "设置炉管转速已下发", "furnace_tube_speed": furnace_tube_speed}
    ...
    def current_furnace_tube_speed(self) -> float:
        """实际炉管转速（读节点 Current_Furnace_Tube_Speed）。"""
        v = self.get_node_value("Current_Furnace_Tube_Speed")
        return v if v is not None else 0.0
