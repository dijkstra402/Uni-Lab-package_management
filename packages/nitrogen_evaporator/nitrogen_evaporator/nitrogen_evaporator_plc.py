"""
氮吹仪 — PLC(OPC UA) 交互驱动 (自动生成，请勿手改)

由 generate_plc_drivers.py 依据 device_templates_actions.csv 生成。
继承 OpcUaClientWithSubscription，把「氮吹仪」标准动作/属性映射为 PLC 节点读写：
...
class NitrogenEvaporatorPLC(OpcUaClientWithSubscription):
    ...
    def nitrogen_blow(self) -> Dict[str, Any]:
        """氮吹：写 Nitrogen_Blow_Trigger 触发，等待 Nitrogen_Blow_Complete 完成后复位。"""
        logger.info("氮吹...")
        self.set_node_value("Nitrogen_Blow_Trigger", True)
        if not self._wait_until_true("Nitrogen_Blow_Complete", description="氮吹完成"):
            raise ValueError("氮吹失败：动作未完成")
        self.set_node_value("Nitrogen_Blow_Trigger", False)
        if not self._wait_until_false("Nitrogen_Blow_Complete", description="氮吹完成复位"):
            raise ValueError("氮吹失败：完成状态复位超时")
        return {"success": True, "message": "氮吹完成"}
    ...
    def station_count(self) -> int:
        """工位数量（读节点 Station_Count）。"""
        v = self.get_node_value("Station_Count")
        return v if v is not None else 0
