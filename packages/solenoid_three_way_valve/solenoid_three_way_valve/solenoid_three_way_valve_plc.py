"""
电磁三通阀 — PLC(OPC UA) 交互驱动 (自动生成，请勿手改)

由 generate_plc_drivers.py 依据 device_templates_actions.csv 生成。
继承 OpcUaClientWithSubscription，把「电磁三通阀」标准动作/属性映射为 PLC 节点读写：
...
class SolenoidThreeWayValvePLC(OpcUaClientWithSubscription):
    ...
    def switch_position(self) -> Dict[str, Any]:
        """切换阀位：写 Switch_Position_Trigger 触发，等待 Switch_Position_Complete 完成后复位。"""
        logger.info("切换阀位...")
        self.set_node_value("Switch_Position_Trigger", True)
        if not self._wait_until_true("Switch_Position_Complete", description="切换阀位完成"):
            raise ValueError("切换阀位失败：动作未完成")
        self.set_node_value("Switch_Position_Trigger", False)
        if not self._wait_until_false("Switch_Position_Complete", description="切换阀位完成复位"):
            raise ValueError("切换阀位失败：完成状态复位超时")
        return {"success": True, "message": "切换阀位完成"}
