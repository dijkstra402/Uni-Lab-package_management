"""
低温或冷冻离心机 — PLC(OPC UA) 交互驱动 (自动生成，请勿手改)

由 generate_plc_drivers.py 依据 device_templates_actions.csv 生成。
继承 OpcUaClientWithSubscription，把「低温或冷冻离心机」标准动作/属性映射为 PLC 节点读写：
...
class RefrigeratedCentrifugePLC(OpcUaClientWithSubscription):
    ...
    def centrifuge_time(self) -> float:
        """离心时间（读节点 Centrifuge_Time）。"""
        v = self.get_node_value("Centrifuge_Time")
        return v if v is not None else 0.0
