...
定义「低温或冷冻离心机」这一设备**大类**的标准动作(`@action`)与状态属性(`@property`)。
...
└── refrigerated_centrifuge/
    ├── __init__.py
    └── refrigerated_centrifuge.py        # 低温或冷冻离心机 标准类定义
```

## 本地验证

```bash
unilab --check_mode --devices ./refrigerated_centrifuge --external_devices_only
```
