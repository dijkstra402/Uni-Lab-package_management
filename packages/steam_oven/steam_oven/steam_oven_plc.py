"""
蒸箱 — PLC(OPC UA) 交互驱动 (自动生成，请勿手改)

由 generate_plc_drivers.py 依据 device_templates_actions.csv 生成。
继承 OpcUaClientWithSubscription，把「蒸箱」标准动作/属性映射为 PLC 节点读写：
...
@device(
    id="steam_oven_plc",
    category=["加热、制冷及空气净化与调节设备", "恒温箱及类似设备", "高温箱", "蒸箱"],
    description="蒸箱 PLC(OPC UA) 交互驱动：动作映射为节点读写。",
    displayname="蒸箱(PLC)",
)
class SteamOvenPLC(OpcUaClientWithSubscription):
    ...
    def set_steaming_time(self, steaming_time: float = 0.0) -> Dict[str, Any]:
        """设置蒸煮时间：写入设定值节点 Steaming_Time_Setpoint。"""
        self.set_node_value("Steaming_Time_Setpoint", steaming_time)
        return {"success": True, "message": "设置蒸煮时间已下发", "steaming_time": steaming_time}

    @action(description="设置蒸汽量")
    def set_steam_volume(self, steam_volume: int = 0) -> Dict[str, Any]:
        """设置蒸汽量：写入设定值节点 Steam_Volume_Setpoint。"""
        self.set_node_value("Steam_Volume_Setpoint", steam_volume)
        return {"success": True, "message": "设置蒸汽量已下发", "steam_volume": steam_volume}
    ...
    def remaining_steaming_time(self) -> float:
        """剩余蒸煮时间（读节点 Remaining_Steaming_Time）。"""
        v = self.get_node_value("Remaining_Steaming_Time")
        return v if v is not None else 0.0

    @property
    @topic_config()
    def current_steam_volume(self) -> int:
        """实际蒸汽量（读节点 Current_Steam_Volume）。"""
        v = self.get_node_value("Current_Steam_Volume")
        return v if v is not None else 0
    ...
    def steam_temperature(self) -> float:
        """蒸汽温度监测（读节点 Steam_Temperature）。"""
        v = self.get_node_value("Steam_Temperature")
        return v if v is not None else 0.0
