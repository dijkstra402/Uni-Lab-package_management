"""
砂浴 — PLC(OPC UA) 交互驱动 (自动生成，请勿手改)

由 generate_plc_drivers.py 依据 device_templates_actions.csv 生成。
继承 OpcUaClientWithSubscription，把「砂浴」标准动作/属性映射为 PLC 节点读写：
...
@device(
    ...
    description="砂浴 PLC(OPC UA) 交互驱动：动作映射为节点读写。",
    displayname="砂浴(PLC)",
)
class SandBathPLC(OpcUaClientWithSubscription):
    ...
    def control_ramp_rate(self) -> Dict[str, Any]:
        """升温速率控制：写 Control_Ramp_Rate_Trigger 触发，等待 Control_Ramp_Rate_Complete 完成后复位。"""
        logger.info("升温速率控制...")
        self.set_node_value("Control_Ramp_Rate_Trigger", True)
        if not self._wait_until_true("Control_Ramp_Rate_Complete", description="升温速率控制完成"):
            raise ValueError("升温速率控制失败：动作未完成")
        self.set_node_value("Control_Ramp_Rate_Trigger", False)
        if not self._wait_until_false("Control_Ramp_Rate_Complete", description="升温速率控制完成复位"):
            raise ValueError("升温速率控制失败：完成状态复位超时")
        return {"success": True, "message": "升温速率控制完成"}
    ...
    def stir_speed(self) -> float:
        """搅拌速度（读节点 Stir_Speed）。"""
        v = self.get_node_value("Stir_Speed")
        return v if v is not None else 0.0
    ...
    def preheat_time(self) -> float:
        """预热时间（读节点 Preheat_Time）。"""
        v = self.get_node_value("Preheat_Time")
        return v if v is not None else 0.0

    @property
    @topic_config()
    def holding_time(self) -> float:
        """保温时间（读节点 Holding_Time）。"""
        v = self.get_node_value("Holding_Time")
        return v if v is not None else 0.0
