"""
复合型清洗机 — PLC(OPC UA) 交互驱动 (自动生成，请勿手改)

由 generate_plc_drivers.py 依据 device_templates_actions.csv 生成。
继承 OpcUaClientWithSubscription，把「复合型清洗机」标准动作/属性映射为 PLC 节点读写：
...
class CompositeCleanerPLC(OpcUaClientWithSubscription):
    ...
    def set_main_wash_power(self, main_wash_power: float = 0.0) -> Dict[str, Any]:
        """设置主清洗功率：写入设定值节点 Main_Wash_Power_Setpoint。"""
        self.set_node_value("Main_Wash_Power_Setpoint", main_wash_power)
        return {"success": True, "message": "设置主清洗功率已下发", "main_wash_power": main_wash_power}

    @action(description="设置辅助清洗功率")
    def set_aux_wash_power(self, aux_wash_power: float = 0.0) -> Dict[str, Any]:
        """设置辅助清洗功率：写入设定值节点 Aux_Wash_Power_Setpoint。"""
        self.set_node_value("Aux_Wash_Power_Setpoint", aux_wash_power)
        return {"success": True, "message": "设置辅助清洗功率已下发", "aux_wash_power": aux_wash_power}

    @action(description="设置总清洗时间")
    def set_total_wash_time(self, total_wash_time: float = 0.0) -> Dict[str, Any]:
        """设置总清洗时间：写入设定值节点 Total_Wash_Time_Setpoint。"""
        self.set_node_value("Total_Wash_Time_Setpoint", total_wash_time)
        return {"success": True, "message": "设置总清洗时间已下发", "total_wash_time": total_wash_time}
    ...
    def modules_sync_state(self) -> bool:
        """各模块协同状态（读节点 Modules_Sync_State）。"""
        v = self.get_node_value("Modules_Sync_State")
        return v if v is not None else False
    ...
    def main_wash_power_feedback(self) -> float:
        """主清洗功率反馈（读节点 Main_Wash_Power_Feedback）。"""
        v = self.get_node_value("Main_Wash_Power_Feedback")
        return v if v is not None else 0.0

    @property
    @topic_config()
    def aux_wash_power_feedback(self) -> float:
        """辅助清洗功率反馈（读节点 Aux_Wash_Power_Feedback）。"""
        v = self.get_node_value("Aux_Wash_Power_Feedback")
        return v if v is not None else 0.0

    @property
    @topic_config()
    def total_wash_time_feedback(self) -> float:
        """总清洗时间反馈（读节点 Total_Wash_Time_Feedback）。"""
        v = self.get_node_value("Total_Wash_Time_Feedback")
        return v if v is not None else 0.0
