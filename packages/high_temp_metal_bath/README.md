...
定义「高温金属浴」这一设备**大类**的标准动作(`@action`)与状态属性(`@property`)。
...
```
high_temp_metal_bath/
├── pyproject.toml
├── requirements.txt
├── .github/workflows/check_registry.yml
└── high_temp_metal_bath/
    ├── __init__.py
    └── high_temp_metal_bath.py        # 高温金属浴 标准类定义
```

## 本地验证

```bash
unilab --check_mode --devices ./high_temp_metal_bath --external_devices_only
```
