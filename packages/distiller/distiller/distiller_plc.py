"""
蒸馏器 — PLC(OPC UA) 交互驱动 (自动生成，请勿手改)

由 generate_plc_drivers.py 依据 device_templates_actions.csv 生成。
继承 OpcUaClientWithSubscription，把「蒸馏器」标准动作/属性映射为 PLC 节点读写：
...
@device(
    id="distiller_plc",
    category=["样品处理仪器与设备", "浓缩设备", "蒸馏器"],
    description="蒸馏器 PLC(OPC UA) 交互驱动：动作映射为节点读写。",
    displayname="蒸馏器(PLC)",
)
class DistillerPLC(OpcUaClientWithSubscription):
    ...
    def distillation_flow(self) -> float:
        """蒸馏流量（读节点 Distillation_Flow）。"""
        v = self.get_node_value("Distillation_Flow")
        return v if v is not None else 0.0
