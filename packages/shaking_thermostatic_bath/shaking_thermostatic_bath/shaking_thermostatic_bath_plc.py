"""
振荡恒温槽 — PLC(OPC UA) 交互驱动 (自动生成，请勿手改)

由 generate_plc_drivers.py 依据 device_templates_actions.csv 生成。
继承 OpcUaClientWithSubscription，把「振荡恒温槽」标准动作/属性映射为 PLC 节点读写：
...
class ShakingThermostaticBathPLC(OpcUaClientWithSubscription):
    ...
    def set_temp_control_precision(self, temp_control_precision: int = 0) -> Dict[str, Any]:
        """设置控温精度：写入设定值节点 Temp_Control_Precision_Setpoint。"""
        self.set_node_value("Temp_Control_Precision_Setpoint", temp_control_precision)
        return {"success": True, "message": "设置控温精度已下发", "temp_control_precision": temp_control_precision}
    ...
    def set_oscillation_frequency(self, oscillation_frequency: float = 0.0) -> Dict[str, Any]:
        """设置振荡频率：写入设定值节点 Oscillation_Frequency_Setpoint。"""
        self.set_node_value("Oscillation_Frequency_Setpoint", oscillation_frequency)
        return {"success": True, "message": "设置振荡频率已下发", "oscillation_frequency": oscillation_frequency}

    @action(description="设置振荡幅度")
    def set_oscillation_amplitude(self, oscillation_amplitude: int = 0) -> Dict[str, Any]:
        """设置振荡幅度：写入设定值节点 Oscillation_Amplitude_Setpoint。"""
        self.set_node_value("Oscillation_Amplitude_Setpoint", oscillation_amplitude)
        return {"success": True, "message": "设置振荡幅度已下发", "oscillation_amplitude": oscillation_amplitude}

    @action(description="设置振荡时间")
    def set_oscillation_time(self, oscillation_time: float = 0.0) -> Dict[str, Any]:
        """设置振荡时间：写入设定值节点 Oscillation_Time_Setpoint。"""
        self.set_node_value("Oscillation_Time_Setpoint", oscillation_time)
        return {"success": True, "message": "设置振荡时间已下发", "oscillation_time": oscillation_time}

    @action(description="设置循环泵转速")
    def set_circulation_pump_speed(self, circulation_pump_speed: float = 0.0) -> Dict[str, Any]:
        """设置循环泵转速：写入设定值节点 Circulation_Pump_Speed_Setpoint。"""
        self.set_node_value("Circulation_Pump_Speed_Setpoint", circulation_pump_speed)
        return {"success": True, "message": "设置循环泵转速已下发", "circulation_pump_speed": circulation_pump_speed}
    ...
    def current_oscillation_frequency(self) -> float:
        """实际振荡频率（读节点 Current_Oscillation_Frequency）。"""
        v = self.get_node_value("Current_Oscillation_Frequency")
        return v if v is not None else 0.0

    @property
    @topic_config()
    def current_oscillation_amplitude(self) -> int:
        """实际振荡幅度（读节点 Current_Oscillation_Amplitude）。"""
        v = self.get_node_value("Current_Oscillation_Amplitude")
        return v if v is not None else 0

    @property
    @topic_config()
    def remaining_oscillation_time(self) -> float:
        """剩余振荡时间（读节点 Remaining_Oscillation_Time）。"""
        v = self.get_node_value("Remaining_Oscillation_Time")
        return v if v is not None else 0.0

    @property
    @topic_config()
    def current_circulation_pump_speed(self) -> float:
        """实际循环泵转速（读节点 Current_Circulation_Pump_Speed）。"""
        v = self.get_node_value("Current_Circulation_Pump_Speed")
        return v if v is not None else 0.0

    @property
    @topic_config()
    def liquid_level(self) -> float:
        """液位监测（读节点 Liquid_Level）。"""
    ...
    def pressure(self) -> float:
        """压力监测（读节点 Pressure）。"""
        v = self.get_node_value("Pressure")
        return v if v is not None else 0.0
