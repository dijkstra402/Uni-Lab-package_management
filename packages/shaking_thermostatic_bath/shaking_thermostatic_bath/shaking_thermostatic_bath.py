"""
振荡恒温槽 — 标准设备类模板 (Device Class Template)

定义「振荡恒温槽」这一设备大类的标准动作(action)与状态属性(property)。
...
@device(
    id="shaking_thermostatic_bath",
    category=["振荡恒温槽"],
    description="振荡恒温槽标准接口：同一大类跨品牌统一的动作与参数。",
    display_name="振荡恒温槽",
)
class ShakingThermostaticBath:
    ...
    def __init__(self, device_id: Optional[str] = None, config: Optional[Dict[str, Any]] = None, **kwargs):
        ...
        Args:
            ...
            config[设备配置]: 设备启动配置。
        """
        self.device_id = device_id or "shaking_thermostatic_bath"
    ...
    def set_oscillation_amplitude(self, oscillation_amplitude: int = 0) -> Dict[str, Any]:
        ...
        Args:
            oscillation_amplitude[振荡幅度]: 目标振荡幅度（单位依设备量程而定）。
        """
        pass
    ...
    def set_oscillation_time(self, oscillation_time: float = 0.0) -> Dict[str, Any]:
        ...
        Args:
            oscillation_time[振荡时间]: 目标振荡时间（单位依设备量程而定）。
        """
        pass
    ...
    def current_oscillation_amplitude(self) -> int:
        """实际振荡幅度。"""
        return self.data.get("current_oscillation_amplitude", 0)

    @property
    @topic_config()
    def remaining_oscillation_time(self) -> float:
        """剩余振荡时间。"""
        return self.data.get("remaining_oscillation_time", 0.0)
