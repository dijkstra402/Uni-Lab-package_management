"""
喷雾沉淀装置 — PLC(OPC UA) 交互驱动 (自动生成，请勿手改)

由 generate_plc_drivers.py 依据 device_templates_actions.csv 生成。
继承 OpcUaClientWithSubscription，把「喷雾沉淀装置」标准动作/属性映射为 PLC 节点读写：
...
class SprayPrecipitationUnitPLC(OpcUaClientWithSubscription):
    ...
    def precipitate(self) -> Dict[str, Any]:
        """沉淀：写 Precipitate_Trigger 触发，等待 Precipitate_Complete 完成后复位。"""
        logger.info("沉淀...")
        self.set_node_value("Precipitate_Trigger", True)
        if not self._wait_until_true("Precipitate_Complete", description="沉淀完成"):
            raise ValueError("沉淀失败：动作未完成")
        self.set_node_value("Precipitate_Trigger", False)
        if not self._wait_until_false("Precipitate_Complete", description="沉淀完成复位"):
            raise ValueError("沉淀失败：完成状态复位超时")
        return {"success": True, "message": "沉淀完成"}

    @action(description="喷雾")
    def spray(self) -> Dict[str, Any]:
        """喷雾：写 Spray_Trigger 触发，等待 Spray_Complete 完成后复位。"""
        logger.info("喷雾...")
        self.set_node_value("Spray_Trigger", True)
        if not self._wait_until_true("Spray_Complete", description="喷雾完成"):
            raise ValueError("喷雾失败：动作未完成")
        self.set_node_value("Spray_Trigger", False)
        if not self._wait_until_false("Spray_Complete", description="喷雾完成复位"):
            raise ValueError("喷雾失败：完成状态复位超时")
        return {"success": True, "message": "喷雾完成"}
    ...
    def liquid_level_alarm(self) -> bool:
        """液位报警（读节点 Liquid_Level_Alarm）。"""
        v = self.get_node_value("Liquid_Level_Alarm")
        return v if v is not None else False
    ...
    def spray_flow(self) -> float:
        """喷雾流量（读节点 Spray_Flow）。"""
        v = self.get_node_value("Spray_Flow")
        return v if v is not None else 0.0

    @property
    @topic_config()
    def spray_pressure(self) -> float:
        """喷雾压力（读节点 Spray_Pressure）。"""
        v = self.get_node_value("Spray_Pressure")
        return v if v is not None else 0.0
