"""
喷雾干燥机 — 标准设备类模板 (Device Class Template)
...
class SprayDryer:
    ...
    def __init__(self, device_id: Optional[str] = None, config: Optional[Dict[str, Any]] = None, **kwargs):
        ...
        Args:
            ...
            config[设备配置]: 设备启动配置。
        """
        self.device_id = device_id or "spray_dryer"
    ...
    def feed_flow(self) -> float:
        """进料流量。"""
        return self.data.get("feed_flow", 0.0)

    @property
    @topic_config()
    def drying_time(self) -> float:
        """干燥时间。"""
        return self.data.get("drying_time", 0.0)

    @property
    @topic_config()
    def collection_state(self) -> int:
        """收料状态。"""
        return self.data.get("collection_state", 0)
