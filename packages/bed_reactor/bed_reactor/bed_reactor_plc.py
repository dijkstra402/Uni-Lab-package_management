"""
床层反应器 — PLC(OPC UA) 交互驱动 (自动生成，请勿手改)

由 generate_plc_drivers.py 依据 device_templates_actions.csv 生成。
继承 OpcUaClientWithSubscription，把「床层反应器」标准动作/属性映射为 PLC 节点读写：
...
@device(
    id="bed_reactor_plc",
    category=["合成制备仪器与设备", "反应器", "床层反应器"],
    description="床层反应器 PLC(OPC UA) 交互驱动：动作映射为节点读写。",
    displayname="床层反应器(PLC)",
)
class BedReactorPLC(OpcUaClientWithSubscription):
