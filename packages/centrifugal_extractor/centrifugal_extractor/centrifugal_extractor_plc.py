"""
离心萃取机 — PLC(OPC UA) 交互驱动 (自动生成，请勿手改)

由 generate_plc_drivers.py 依据 device_templates_actions.csv 生成。
继承 OpcUaClientWithSubscription，把「离心萃取机」标准动作/属性映射为 PLC 节点读写：
...
class CentrifugalExtractorPLC(OpcUaClientWithSubscription):
    ...
    def set_separation_time(self, separation_time: float = 0.0) -> Dict[str, Any]:
        """设置分离时间：写入设定值节点 Separation_Time_Setpoint。"""
        self.set_node_value("Separation_Time_Setpoint", separation_time)
        return {"success": True, "message": "设置分离时间已下发", "separation_time": separation_time}

    @action(description="设置进料流量")
    def set_feed_flow(self, feed_flow: float = 0.0) -> Dict[str, Any]:
        """设置进料流量：写入设定值节点 Feed_Flow_Setpoint。"""
        self.set_node_value("Feed_Flow_Setpoint", feed_flow)
        return {"success": True, "message": "设置进料流量已下发", "feed_flow": feed_flow}

    @action(description="设置轻重相界面")
    def set_phase_interface(self, phase_interface: int = 0) -> Dict[str, Any]:
        """设置轻重相界面：写入设定值节点 Phase_Interface_Setpoint。"""
        self.set_node_value("Phase_Interface_Setpoint", phase_interface)
        return {"success": True, "message": "设置轻重相界面已下发", "phase_interface": phase_interface}
    ...
    def centrifuge_on_state(self) -> bool:
        """离心开启状态（读节点 Centrifuge_On_State）。"""
        v = self.get_node_value("Centrifuge_On_State")
        return v if v is not None else False

    @property
    @topic_config()
    def feeding_state(self) -> bool:
        """进料状态（读节点 Feeding_State）。"""
        v = self.get_node_value("Feeding_State")
        return v if v is not None else False
    ...
    def current_separation_time(self) -> float:
        """实际分离时间（读节点 Current_Separation_Time）。"""
        v = self.get_node_value("Current_Separation_Time")
        return v if v is not None else 0.0

    @property
    @topic_config()
    def current_feed_flow(self) -> float:
        """实际进料流量（读节点 Current_Feed_Flow）。"""
        v = self.get_node_value("Current_Feed_Flow")
        return v if v is not None else 0.0

    @property
    @topic_config()
    def current_phase_interface(self) -> int:
        """实际轻重相界面（读节点 Current_Phase_Interface）。"""
        v = self.get_node_value("Current_Phase_Interface")
        return v if v is not None else 0
