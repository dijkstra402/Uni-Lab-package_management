"""
抽提萃取设备 — PLC(OPC UA) 交互驱动 (自动生成，请勿手改)

由 generate_plc_drivers.py 依据 device_templates_actions.csv 生成。
继承 OpcUaClientWithSubscription，把「抽提萃取设备」标准动作/属性映射为 PLC 节点读写：
...
@device(
    id="solvent_extractor_plc",
    category=["样品处理仪器与设备", "提取设备", "抽提萃取设备"],
    description="抽提萃取设备 PLC(OPC UA) 交互驱动：动作映射为节点读写。",
    displayname="抽提萃取设备(PLC)",
)
class SolventExtractorPLC(OpcUaClientWithSubscription):
    ...
    def set_condensate_flow(self, condensate_flow: float = 0.0) -> Dict[str, Any]:
        """设置冷凝水流量：写入设定值节点 Condensate_Flow_Setpoint。"""
        self.set_node_value("Condensate_Flow_Setpoint", condensate_flow)
        return {"success": True, "message": "设置冷凝水流量已下发", "condensate_flow": condensate_flow}

    @action(description="设置回流时间")
    def set_reflux_time(self, reflux_time: float = 0.0) -> Dict[str, Any]:
        """设置回流时间：写入设定值节点 Reflux_Time_Setpoint。"""
        self.set_node_value("Reflux_Time_Setpoint", reflux_time)
        return {"success": True, "message": "设置回流时间已下发", "reflux_time": reflux_time}
    ...
    def heating_state(self) -> bool:
        """加热状态（读节点 Heating_State）。"""
        v = self.get_node_value("Heating_State")
        return v if v is not None else False

    @property
    @topic_config()
    def condensation_state(self) -> bool:
        """冷凝状态（读节点 Condensation_State）。"""
        v = self.get_node_value("Condensation_State")
        return v if v is not None else False

    @property
    @topic_config()
    def reflux_state(self) -> bool:
        """回流状态（读节点 Reflux_State）。"""
        v = self.get_node_value("Reflux_State")
        return v if v is not None else False
    ...
    def current_heating_temperature(self) -> float:
        """实际加热温度（读节点 Current_Heating_Temperature）。"""
        v = self.get_node_value("Current_Heating_Temperature")
        return v if v is not None else 0.0

    @property
    @topic_config()
    def current_condensate_flow(self) -> float:
        """实际冷凝水流量（读节点 Current_Condensate_Flow）。"""
        v = self.get_node_value("Current_Condensate_Flow")
        return v if v is not None else 0.0

    @property
    @topic_config()
    def current_reflux_time(self) -> float:
        """实际回流时间（读节点 Current_Reflux_Time）。"""
        v = self.get_node_value("Current_Reflux_Time")
        return v if v is not None else 0.0
