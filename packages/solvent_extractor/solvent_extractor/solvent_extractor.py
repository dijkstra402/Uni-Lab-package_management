"""
抽提萃取设备 — 标准设备类模板 (Device Class Template)

定义「抽提萃取设备」这一设备大类的标准动作(action)与状态属性(property)。
...
@device(
    id="solvent_extractor",
    category=["样品处理仪器与设备", "提取设备", "抽提萃取设备"],
    description="抽提萃取设备标准接口：同一大类跨品牌统一的动作与参数。",
    displayname="抽提萃取设备",
)
class SolventExtractor:
    ...
    def __init__(self, device_id: Optional[str] = None, config: Optional[Dict[str, Any]] = None, **kwargs):
        ...
        Args:
            ...
            config[设备配置]: 设备启动配置。
        """
        self.device_id = device_id or "solvent_extractor"
    ...
    def set_condensate_flow(self, condensate_flow: float = 0.0) -> Dict[str, Any]:
        ...
        Args:
            condensate_flow[冷凝水流量]: 目标冷凝水流量（单位依设备量程而定）。
        """
        raise NotImplementedError("请在设备包中实现该动作")
    ...
    def condensation_state(self) -> bool:
        """冷凝状态。"""
        return self.data.get("condensation_state", False)
    ...
    def current_condensate_flow(self) -> float:
        """实际冷凝水流量。"""
        return self.data.get("current_condensate_flow", 0.0)
