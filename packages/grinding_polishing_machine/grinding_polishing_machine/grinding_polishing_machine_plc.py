"""
磨抛机 — PLC(OPC UA) 交互驱动 (自动生成，请勿手改)

由 generate_plc_drivers.py 依据 device_templates_actions.csv 生成。
继承 OpcUaClientWithSubscription，把「磨抛机」标准动作/属性映射为 PLC 节点读写：
...
@device(
    id="grinding_polishing_machine_plc",
    category=["磨抛机"],
    description="磨抛机 PLC(OPC UA) 交互驱动：动作映射为节点读写。",
    display_name="磨抛机(PLC)",
)
class GrindingPolishingMachinePLC(OpcUaClientWithSubscription):
    ...
    def set_polishing_speed(self, polishing_speed: float = 0.0) -> Dict[str, Any]:
        """设置抛光转速：写入设定值节点 Polishing_Speed_Setpoint。"""
        self.set_node_value("Polishing_Speed_Setpoint", polishing_speed)
        return {"success": True, "message": "设置抛光转速已下发", "polishing_speed": polishing_speed}
    ...
    def set_polishing_pressure(self, polishing_pressure: float = 0.0) -> Dict[str, Any]:
        """设置磨抛压力：写入设定值节点 Polishing_Pressure_Setpoint。"""
        self.set_node_value("Polishing_Pressure_Setpoint", polishing_pressure)
        return {"success": True, "message": "设置磨抛压力已下发", "polishing_pressure": polishing_pressure}

    @action(description="设置样品夹持力")
    def set_sample_clamp_force(self, sample_clamp_force: int = 0) -> Dict[str, Any]:
        """设置样品夹持力：写入设定值节点 Sample_Clamp_Force_Setpoint。"""
        self.set_node_value("Sample_Clamp_Force_Setpoint", sample_clamp_force)
        return {"success": True, "message": "设置样品夹持力已下发", "sample_clamp_force": sample_clamp_force}

    @action(description="设置磨抛精度")
    def set_polishing_precision(self, polishing_precision: int = 0) -> Dict[str, Any]:
        """设置磨抛精度：写入设定值节点 Polishing_Precision_Setpoint。"""
        self.set_node_value("Polishing_Precision_Setpoint", polishing_precision)
        return {"success": True, "message": "设置磨抛精度已下发", "polishing_precision": polishing_precision}
