...
定义「电磁开关阀」这一设备**大类**的标准动作(`@action`)与状态属性(`@property`)。
...
```
solenoid_on_off_valve/
├── pyproject.toml
├── requirements.txt
├── .github/workflows/check_registry.yml
└── solenoid_on_off_valve/
    ├── __init__.py
    └── solenoid_on_off_valve.py        # 电磁开关阀 标准类定义
```

## 本地验证

```bash
unilab --check_mode --devices ./solenoid_on_off_valve --external_devices_only
```
