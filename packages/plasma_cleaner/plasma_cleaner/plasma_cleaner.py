...
class PlasmaCleaner:
    ...
    @property
    @topic_config()
    def plasma_power_feedback(self) -> int:
        """等离子功率反馈。"""
        return self.data.get("plasma_power_feedback", 0)

    @property
    @topic_config()
    def vacuum_feedback(self) -> int:
        """真空度反馈。"""
        return self.data.get("vacuum_feedback", 0)

    @property
    @topic_config()
    def treatment_time_feedback(self) -> int:
        """处理时间反馈。"""
        return self.data.get("treatment_time_feedback", 0)

    @property
    @topic_config()
    def gas_flow_feedback(self) -> int:
        """气体流量反馈。"""
        return self.data.get("gas_flow_feedback", 0)
