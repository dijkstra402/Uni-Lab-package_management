"""
等离子体清洗机 — PLC(OPC UA) 交互驱动 (自动生成，请勿手改)

由 generate_plc_drivers.py 依据 device_templates_actions.csv 生成。
继承 OpcUaClientWithSubscription，把「等离子体清洗机」标准动作/属性映射为 PLC 节点读写：
...
@device(
    id="plasma_cleaner_plc",
    category=["样品处理仪器与设备", "清洗机", "等离子体清洗机"],
    description="等离子体清洗机 PLC(OPC UA) 交互驱动：动作映射为节点读写。",
    displayname="等离子体清洗机(PLC)",
)
class PlasmaCleanerPLC(OpcUaClientWithSubscription):
    ...
    def set_plasma_power(self, plasma_power: float = 0.0) -> Dict[str, Any]:
        """设置等离子功率：写入设定值节点 Plasma_Power_Setpoint。"""
        self.set_node_value("Plasma_Power_Setpoint", plasma_power)
        return {"success": True, "message": "设置等离子功率已下发", "plasma_power": plasma_power}
    ...
    def plasma_power_feedback(self) -> float:
        """等离子功率反馈（读节点 Plasma_Power_Feedback）。"""
        v = self.get_node_value("Plasma_Power_Feedback")
        return v if v is not None else 0.0

    @property
    @topic_config()
    def vacuum_feedback(self) -> float:
        """真空度反馈（读节点 Vacuum_Feedback）。"""
        v = self.get_node_value("Vacuum_Feedback")
        return v if v is not None else 0.0

    @property
    @topic_config()
    def treatment_time_feedback(self) -> float:
        """处理时间反馈（读节点 Treatment_Time_Feedback）。"""
        v = self.get_node_value("Treatment_Time_Feedback")
        return v if v is not None else 0.0

    @property
    @topic_config()
    def gas_flow_feedback(self) -> float:
        """气体流量反馈（读节点 Gas_Flow_Feedback）。"""
        v = self.get_node_value("Gas_Flow_Feedback")
        return v if v is not None else 0.0
