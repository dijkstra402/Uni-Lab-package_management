"""
高低温金属浴 — PLC(OPC UA) 交互驱动 (自动生成，请勿手改)

由 generate_plc_drivers.py 依据 device_templates_actions.csv 生成。
继承 OpcUaClientWithSubscription，把「高低温金属浴」标准动作/属性映射为 PLC 节点读写：
...
class HighLowTempMetalBathPLC(OpcUaClientWithSubscription):
    ...
    @action(description="加热/制冷启动")
    def start_heat_cool(self) -> Dict[str, Any]:
        """加热/制冷启动：写 Start_Heat_Cool_Trigger 触发，等待 Start_Heat_Cool_Complete 完成后复位。"""
        logger.info("加热/制冷启动...")
        self.set_node_value("Start_Heat_Cool_Trigger", True)
        if not self._wait_until_true("Start_Heat_Cool_Complete", description="加热/制冷启动完成"):
            raise ValueError("加热/制冷启动失败：动作未完成")
        self.set_node_value("Start_Heat_Cool_Trigger", False)
        if not self._wait_until_false("Start_Heat_Cool_Complete", description="加热/制冷启动完成复位"):
            raise ValueError("加热/制冷启动失败：完成状态复位超时")
        return {"success": True, "message": "加热/制冷启动完成"}
