"""
蒸汽清洗机 — PLC(OPC UA) 交互驱动 (自动生成，请勿手改)

由 generate_plc_drivers.py 依据 device_templates_actions.csv 生成。
继承 OpcUaClientWithSubscription，把「蒸汽清洗机」标准动作/属性映射为 PLC 节点读写：
...
@device(
    id="steam_cleaner_plc",
    category=["样品处理仪器与设备", "清洗机", "蒸汽清洗机"],
    description="蒸汽清洗机 PLC(OPC UA) 交互驱动：动作映射为节点读写。",
    displayname="蒸汽清洗机(PLC)",
)
class SteamCleanerPLC(OpcUaClientWithSubscription):
    ...
    def set_steam_pressure(self, steam_pressure: float = 0.0) -> Dict[str, Any]:
        """设置蒸汽压力：写入设定值节点 Steam_Pressure_Setpoint。"""
        self.set_node_value("Steam_Pressure_Setpoint", steam_pressure)
        return {"success": True, "message": "设置蒸汽压力已下发", "steam_pressure": steam_pressure}

    @action(description="设置蒸汽温度")
    def set_steam_temperature(self, steam_temperature: float = 0.0) -> Dict[str, Any]:
        """设置蒸汽温度：写入设定值节点 Steam_Temperature_Setpoint。"""
        self.set_node_value("Steam_Temperature_Setpoint", steam_temperature)
        return {"success": True, "message": "设置蒸汽温度已下发", "steam_temperature": steam_temperature}

    @action(description="设置蒸汽时间")
    def set_steam_time(self, steam_time: float = 0.0) -> Dict[str, Any]:
        """设置蒸汽时间：写入设定值节点 Steam_Time_Setpoint。"""
        self.set_node_value("Steam_Time_Setpoint", steam_time)
        return {"success": True, "message": "设置蒸汽时间已下发", "steam_time": steam_time}
    ...
    def steam_generator_state(self) -> bool:
        """蒸汽发生器状态（读节点 Steam_Generator_State）。"""
        v = self.get_node_value("Steam_Generator_State")
        return v if v is not None else False

    @property
    @topic_config()
    def drain_valve_state(self) -> bool:
        """疏水阀状态（读节点 Drain_Valve_State）。"""
        v = self.get_node_value("Drain_Valve_State")
        return v if v is not None else False
    ...
    def steam_pressure_feedback(self) -> float:
        """蒸汽压力反馈（读节点 Steam_Pressure_Feedback）。"""
        v = self.get_node_value("Steam_Pressure_Feedback")
        return v if v is not None else 0.0

    @property
    @topic_config()
    def steam_temperature_feedback(self) -> float:
        """蒸汽温度反馈（读节点 Steam_Temperature_Feedback）。"""
        v = self.get_node_value("Steam_Temperature_Feedback")
        return v if v is not None else 0.0

    @property
    @topic_config()
    def steam_time_feedback(self) -> float:
        """蒸汽时间反馈（读节点 Steam_Time_Feedback）。"""
        v = self.get_node_value("Steam_Time_Feedback")
        return v if v is not None else 0.0
