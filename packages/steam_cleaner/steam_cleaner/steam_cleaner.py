"""
蒸汽清洗机 — 标准设备类模板 (Device Class Template)

定义「蒸汽清洗机」这一设备大类的标准动作(action)与状态属性(property)。
...
@device(
    ...
    description="蒸汽清洗机标准接口：同一大类跨品牌统一的动作与参数。",
    displayname="蒸汽清洗机",
)
class SteamCleaner:
    ...
    def __init__(self, device_id: Optional[str] = None, config: Optional[Dict[str, Any]] = None, **kwargs):
        ...
        Args:
            ...
            config[设备配置]: 设备启动配置。
        """
        self.device_id = device_id or "steam_cleaner"
    ...
    def drain_valve_state(self) -> bool:
        """疏水阀状态。"""
        return self.data.get("drain_valve_state", False)
