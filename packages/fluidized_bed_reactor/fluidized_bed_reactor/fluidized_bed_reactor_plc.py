"""
流化床反应器 — PLC(OPC UA) 交互驱动 (自动生成，请勿手改)

由 generate_plc_drivers.py 依据 device_templates_actions.csv 生成。
继承 OpcUaClientWithSubscription，把「流化床反应器」标准动作/属性映射为 PLC 节点读写：
...
class FluidizedBedReactorPLC(OpcUaClientWithSubscription):
    ...
    def set_fluidization_wind_speed(self, fluidization_wind_speed: int = 0) -> Dict[str, Any]:
        """设置流化风速：写入设定值节点 Fluidization_Wind_Speed_Setpoint。"""
        self.set_node_value("Fluidization_Wind_Speed_Setpoint", fluidization_wind_speed)
        return {"success": True, "message": "设置流化风速已下发", "fluidization_wind_speed": fluidization_wind_speed}

    @action(description="预热气")
    def preheat_gas(self) -> Dict[str, Any]:
        """预热气：写 Preheat_Gas_Trigger 触发，等待 Preheat_Gas_Complete 完成后复位。"""
        logger.info("预热气...")
        self.set_node_value("Preheat_Gas_Trigger", True)
        if not self._wait_until_true("Preheat_Gas_Complete", description="预热气完成"):
            raise ValueError("预热气失败：动作未完成")
        self.set_node_value("Preheat_Gas_Trigger", False)
        if not self._wait_until_false("Preheat_Gas_Complete", description="预热气完成复位"):
            raise ValueError("预热气失败：完成状态复位超时")
        return {"success": True, "message": "预热气完成"}
    ...
    @property
    @topic_config()
    def current_fluidization_wind_speed(self) -> int:
        """实际流化风速（读节点 Current_Fluidization_Wind_Speed）。"""
        v = self.get_node_value("Current_Fluidization_Wind_Speed")
        return v if v is not None else 0
