...
定义「流化床反应器」这一设备**大类**的标准动作(`@action`)与状态属性(`@property`)。
...
└── fluidized_bed_reactor/
    ├── __init__.py
    └── fluidized_bed_reactor.py        # 流化床反应器 标准类定义
```

## 本地验证

```bash
unilab --check_mode --devices ./fluidized_bed_reactor --external_devices_only
```
