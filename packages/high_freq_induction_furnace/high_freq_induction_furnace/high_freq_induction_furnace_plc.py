"""
高频感应炉 — PLC(OPC UA) 交互驱动 (自动生成，请勿手改)

由 generate_plc_drivers.py 依据 device_templates_actions.csv 生成。
继承 OpcUaClientWithSubscription，把「高频感应炉」标准动作/属性映射为 PLC 节点读写：
...
@device(
    id="high_freq_induction_furnace_plc",
    category=["高频感应炉"],
    description="高频感应炉 PLC(OPC UA) 交互驱动：动作映射为节点读写。",
    display_name="高频感应炉(PLC)",
)
class HighFreqInductionFurnacePLC(OpcUaClientWithSubscription):
    ...
    def set_induction_power(self, induction_power: float = 0.0) -> Dict[str, Any]:
        """设置感应功率：写入设定值节点 Induction_Power_Setpoint。"""
        self.set_node_value("Induction_Power_Setpoint", induction_power)
        return {"success": True, "message": "设置感应功率已下发", "induction_power": induction_power}
    ...
    def set_cooling_water_velocity(self, cooling_water_velocity: float = 0.0) -> Dict[str, Any]:
        """设置冷却水流速：写入设定值节点 Cooling_Water_Velocity_Setpoint。"""
        self.set_node_value("Cooling_Water_Velocity_Setpoint", cooling_water_velocity)
        return {"success": True, "message": "设置冷却水流速已下发", "cooling_water_velocity": cooling_water_velocity}
    ...
    def current_induction_power(self) -> float:
        """实际感应功率（读节点 Current_Induction_Power）。"""
        v = self.get_node_value("Current_Induction_Power")
        return v if v is not None else 0.0
    ...
    def current_cooling_water_velocity(self) -> float:
        """实际冷却水流速（读节点 Current_Cooling_Water_Velocity）。"""
        v = self.get_node_value("Current_Cooling_Water_Velocity")
        return v if v is not None else 0.0
