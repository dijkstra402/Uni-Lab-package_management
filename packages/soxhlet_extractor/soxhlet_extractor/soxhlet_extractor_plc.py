"""
索氏提取仪 — PLC(OPC UA) 交互驱动 (自动生成，请勿手改)

由 generate_plc_drivers.py 依据 device_templates_actions.csv 生成。
继承 OpcUaClientWithSubscription，把「索氏提取仪」标准动作/属性映射为 PLC 节点读写：
...
class SoxhletExtractorPLC(OpcUaClientWithSubscription):
    ...
    def set_extraction_count(self, extraction_count: int = 0) -> Dict[str, Any]:
        """设置提取次数：写入设定值节点 Extraction_Count_Setpoint。"""
        self.set_node_value("Extraction_Count_Setpoint", extraction_count)
        return {"success": True, "message": "设置提取次数已下发", "extraction_count": extraction_count}
    ...
    def solvent_level_state(self) -> bool:
        """溶剂液位状态（读节点 Solvent_Level_State）。"""
        v = self.get_node_value("Solvent_Level_State")
        return v if v is not None else False
    ...
    def current_extraction_count(self) -> int:
        """实际提取次数（读节点 Current_Extraction_Count）。"""
        v = self.get_node_value("Current_Extraction_Count")
        return v if v is not None else 0
