"""
离心机 — PLC(OPC UA) 交互驱动 (自动生成，请勿手改)

由 generate_plc_drivers.py 依据 device_templates_actions.csv 生成。
继承 OpcUaClientWithSubscription，把「离心机」标准动作/属性映射为 PLC 节点读写：
...
@device(
    id="centrifuge_plc",
    category=["样品处理仪器与设备", "分离设备", "离心机"],
    description="离心机 PLC(OPC UA) 交互驱动：动作映射为节点读写。",
    displayname="离心机(PLC)",
)
class CentrifugePLC(OpcUaClientWithSubscription):
    ...
    def set_centrifuge_time(self, centrifuge_time: float = 0.0) -> Dict[str, Any]:
        """设置离心时间：写入设定值节点 Centrifuge_Time_Setpoint。"""
        self.set_node_value("Centrifuge_Time_Setpoint", centrifuge_time)
        return {"success": True, "message": "设置离心时间已下发", "centrifuge_time": centrifuge_time}
    ...
    def centrifuge(self) -> Dict[str, Any]:
        """离心：写 Centrifuge_Trigger 触发，等待 Centrifuge_Complete 完成后复位。"""
        logger.info("离心...")
        self.set_node_value("Centrifuge_Trigger", True)
        if not self._wait_until_true("Centrifuge_Complete", description="离心完成"):
            raise ValueError("离心失败：动作未完成")
        self.set_node_value("Centrifuge_Trigger", False)
        if not self._wait_until_false("Centrifuge_Complete", description="离心完成复位"):
            raise ValueError("离心失败：完成状态复位超时")
        return {"success": True, "message": "离心完成"}
