"""
振荡金属浴 — PLC(OPC UA) 交互驱动 (自动生成，请勿手改)

由 generate_plc_drivers.py 依据 device_templates_actions.csv 生成。
继承 OpcUaClientWithSubscription，把「振荡金属浴」标准动作/属性映射为 PLC 节点读写：
...
@device(
    id="shaking_metal_bath_plc",
    category=["加热、制冷及空气净化与调节设备", "固体浴", "振荡金属浴"],
    description="振荡金属浴 PLC(OPC UA) 交互驱动：动作映射为节点读写。",
    displayname="振荡金属浴(PLC)",
)
class ShakingMetalBathPLC(OpcUaClientWithSubscription):
    ...
    def start_oscillation(self) -> Dict[str, Any]:
        """振荡启动：写 Start_Oscillation_Trigger 触发，等待 Start_Oscillation_Complete 完成后复位。"""
        logger.info("振荡启动...")
        self.set_node_value("Start_Oscillation_Trigger", True)
        if not self._wait_until_true("Start_Oscillation_Complete", description="振荡启动完成"):
            raise ValueError("振荡启动失败：动作未完成")
        self.set_node_value("Start_Oscillation_Trigger", False)
        if not self._wait_until_false("Start_Oscillation_Complete", description="振荡启动完成复位"):
            raise ValueError("振荡启动失败：完成状态复位超时")
        return {"success": True, "message": "振荡启动完成"}
    ...
    def oscillation_speed(self) -> float:
        """振荡速度（读节点 Oscillation_Speed）。"""
        v = self.get_node_value("Oscillation_Speed")
