"""
振荡器（非恒温） — PLC(OPC UA) 交互驱动 (自动生成，请勿手改)

由 generate_plc_drivers.py 依据 device_templates_actions.csv 生成。
继承 OpcUaClientWithSubscription，把「振荡器（非恒温）」标准动作/属性映射为 PLC 节点读写：
...
class ShakerPLC(OpcUaClientWithSubscription):
    ...
    def set_amplitude(self, amplitude: int = 0) -> Dict[str, Any]:
        """设置振幅：写入设定值节点 Amplitude_Setpoint。"""
        self.set_node_value("Amplitude_Setpoint", amplitude)
        return {"success": True, "message": "设置振幅已下发", "amplitude": amplitude}
    ...
    def ultrasonicate(self) -> Dict[str, Any]:
        """超声：写 Ultrasonicate_Trigger 触发，等待 Ultrasonicate_Complete 完成后复位。"""
        logger.info("超声...")
        self.set_node_value("Ultrasonicate_Trigger", True)
        if not self._wait_until_true("Ultrasonicate_Complete", description="超声完成"):
            raise ValueError("超声失败：动作未完成")
        self.set_node_value("Ultrasonicate_Trigger", False)
        if not self._wait_until_false("Ultrasonicate_Complete", description="超声完成复位"):
            raise ValueError("超声失败：完成状态复位超时")
        return {"success": True, "message": "超声完成"}
    ...
    def ultrasonic_time(self) -> float:
        """超声时间（读节点 Ultrasonic_Time）。"""
        v = self.get_node_value("Ultrasonic_Time")
        return v if v is not None else 0.0
