...
定义「熔样机」这一设备**大类**的标准动作(`@action`)与状态属性(`@property`)。
...
```
sample_fusion_machine/
├── pyproject.toml
├── requirements.txt
├── .github/workflows/check_registry.yml
└── sample_fusion_machine/
    ├── __init__.py
    └── sample_fusion_machine.py        # 熔样机 标准类定义
```

## 本地验证

```bash
unilab --check_mode --devices ./sample_fusion_machine --external_devices_only
```
