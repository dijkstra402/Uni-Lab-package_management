"""
打粉机 — 标准设备类模板 (Device Class Template)

定义「打粉机」这一设备大类的标准动作(action)与状态属性(property)。
...
@device(
    ...
    description="打粉机标准接口：同一大类跨品牌统一的动作与参数。",
    displayname="打粉机",
)
class Pulverizer:
    ...
    def __init__(self, device_id: Optional[str] = None, config: Optional[Dict[str, Any]] = None, **kwargs):
        ...
        Args:
            ...
            config[设备配置]: 设备启动配置。
        """
        self.device_id = device_id or "pulverizer"
    ...
    def cooling_temperature(self) -> float:
        """冷却温度。"""
        return self.data.get("cooling_temperature", 0.0)
