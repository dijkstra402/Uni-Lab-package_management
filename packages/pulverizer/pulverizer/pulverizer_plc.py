"""
打粉机 — PLC(OPC UA) 交互驱动 (自动生成，请勿手改)

由 generate_plc_drivers.py 依据 device_templates_actions.csv 生成。
继承 OpcUaClientWithSubscription，把「打粉机」标准动作/属性映射为 PLC 节点读写：
...
@device(
    id="pulverizer_plc",
    category=["样品处理仪器与设备", "粉碎设备", "打粉机"],
    description="打粉机 PLC(OPC UA) 交互驱动：动作映射为节点读写。",
    displayname="打粉机(PLC)",
)
class PulverizerPLC(OpcUaClientWithSubscription):
    ...
    def pulverize(self) -> Dict[str, Any]:
        """打粉：写 Pulverize_Trigger 触发，等待 Pulverize_Complete 完成后复位。"""
        logger.info("打粉...")
        self.set_node_value("Pulverize_Trigger", True)
        if not self._wait_until_true("Pulverize_Complete", description="打粉完成"):
            raise ValueError("打粉失败：动作未完成")
        self.set_node_value("Pulverize_Trigger", False)
        if not self._wait_until_false("Pulverize_Complete", description="打粉完成复位"):
            raise ValueError("打粉失败：完成状态复位超时")
        return {"success": True, "message": "打粉完成"}
    ...
    def cooling_temperature(self) -> float:
        """冷却温度（读节点 Cooling_Temperature）。"""
        v = self.get_node_value("Cooling_Temperature")
        return v if v is not None else 0.0
