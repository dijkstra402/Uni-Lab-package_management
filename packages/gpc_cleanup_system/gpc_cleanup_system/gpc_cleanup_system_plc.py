"""
凝胶渗透净化系统 — PLC(OPC UA) 交互驱动 (自动生成，请勿手改)

由 generate_plc_drivers.py 依据 device_templates_actions.csv 生成。
继承 OpcUaClientWithSubscription，把「凝胶渗透净化系统」标准动作/属性映射为 PLC 节点读写：
...
@device(
    id="gpc_cleanup_system_plc",
    category=["样品处理仪器与设备", "纯化设备", "凝胶渗透净化系统"],
    description="凝胶渗透净化系统 PLC(OPC UA) 交互驱动：动作映射为节点读写。",
    displayname="凝胶渗透净化系统(PLC)",
)
class GpcCleanupSystemPLC(OpcUaClientWithSubscription):
    ...
    def set_mobile_phase_velocity(self, mobile_phase_velocity: float = 0.0) -> Dict[str, Any]:
        """设置流动相流速：写入设定值节点 Mobile_Phase_Velocity_Setpoint。"""
        self.set_node_value("Mobile_Phase_Velocity_Setpoint", mobile_phase_velocity)
        return {"success": True, "message": "设置流动相流速已下发", "mobile_phase_velocity": mobile_phase_velocity}

    @action(description="设置柱温")
    def set_column_temperature(self, column_temperature: int = 0) -> Dict[str, Any]:
        """设置柱温：写入设定值节点 Column_Temperature_Setpoint。"""
        self.set_node_value("Column_Temperature_Setpoint", column_temperature)
        return {"success": True, "message": "设置柱温已下发", "column_temperature": column_temperature}
    ...
    def current_velocity(self) -> float:
        """实际流速（读节点 Current_Velocity）。"""
        v = self.get_node_value("Current_Velocity")
