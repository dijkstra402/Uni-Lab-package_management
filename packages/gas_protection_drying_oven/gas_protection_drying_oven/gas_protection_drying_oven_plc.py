"""
气体保护干燥箱 — PLC(OPC UA) 交互驱动 (自动生成，请勿手改)

由 generate_plc_drivers.py 依据 device_templates_actions.csv 生成。
继承 OpcUaClientWithSubscription，把「气体保护干燥箱」标准动作/属性映射为 PLC 节点读写：
...
@device(
    id="gas_protection_drying_oven_plc",
    category=["加热、制冷及空气净化与调节设备", "恒温箱及类似设备", "高温箱", "气体保护干燥箱"],
    description="气体保护干燥箱 PLC(OPC UA) 交互驱动：动作映射为节点读写。",
    displayname="气体保护干燥箱(PLC)",
)
class GasProtectionDryingOvenPLC(OpcUaClientWithSubscription):
    ...
    @action(description="设置气体浓度")
    def set_gas_concentration(self, gas_concentration: float = 0.0) -> Dict[str, Any]:
        """设置气体浓度：写入设定值节点 Gas_Concentration_Setpoint。"""
        self.set_node_value("Gas_Concentration_Setpoint", gas_concentration)
        return {"success": True, "message": "设置气体浓度已下发", "gas_concentration": gas_concentration}
    ...
    @property
    @topic_config()
    def current_gas_concentration(self) -> float:
        """实际气体浓度（读节点 Current_Gas_Concentration）。"""
        v = self.get_node_value("Current_Gas_Concentration")
        return v if v is not None else 0.0

    @property
    @topic_config()
    def current_gas_flow(self) -> float:
        """实际气体流量（读节点 Current_Gas_Flow）。"""
        v = self.get_node_value("Current_Gas_Flow")
        return v if v is not None else 0.0
