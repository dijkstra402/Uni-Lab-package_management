"""
高压消毒机 — PLC(OPC UA) 交互驱动 (自动生成，请勿手改)

由 generate_plc_drivers.py 依据 device_templates_actions.csv 生成。
继承 OpcUaClientWithSubscription，把「高压消毒机」标准动作/属性映射为 PLC 节点读写：
...
@device(
    id="autoclave_sterilizer_plc",
    category=["高压消毒机"],
    description="高压消毒机 PLC(OPC UA) 交互驱动：动作映射为节点读写。",
    display_name="高压消毒机(PLC)",
)
class AutoclaveSterilizerPLC(OpcUaClientWithSubscription):
    ...
    def set_disinfection_pressure(self, disinfection_pressure: float = 0.0) -> Dict[str, Any]:
        """设置消毒压力：写入设定值节点 Disinfection_Pressure_Setpoint。"""
        self.set_node_value("Disinfection_Pressure_Setpoint", disinfection_pressure)
        return {"success": True, "message": "设置消毒压力已下发", "disinfection_pressure": disinfection_pressure}

    @action(description="设置消毒温度")
    def set_disinfection_temperature(self, disinfection_temperature: float = 0.0) -> Dict[str, Any]:
        """设置消毒温度：写入设定值节点 Disinfection_Temperature_Setpoint。"""
        self.set_node_value("Disinfection_Temperature_Setpoint", disinfection_temperature)
        return {"success": True, "message": "设置消毒温度已下发", "disinfection_temperature": disinfection_temperature}
    ...
    def exhaust_valve_state(self) -> bool:
        """排气阀状态（读节点 Exhaust_Valve_State）。"""
        v = self.get_node_value("Exhaust_Valve_State")
        return v if v is not None else False

    @property
    @topic_config()
    def safety_valve_state(self) -> bool:
        """安全阀状态（读节点 Safety_Valve_State）。"""
        v = self.get_node_value("Safety_Valve_State")
        return v if v is not None else False

    @property
    @topic_config()
    def drying_function_state(self) -> bool:
        """干燥功能状态（读节点 Drying_Function_State）。"""
        v = self.get_node_value("Drying_Function_State")
        return v if v is not None else False
    ...
    def disinfection_pressure_feedback(self) -> float:
        """消毒压力反馈（读节点 Disinfection_Pressure_Feedback）。"""
        v = self.get_node_value("Disinfection_Pressure_Feedback")
        return v if v is not None else 0.0

    @property
    @topic_config()
    def disinfection_temperature_feedback(self) -> float:
        """消毒温度反馈（读节点 Disinfection_Temperature_Feedback）。"""
        v = self.get_node_value("Disinfection_Temperature_Feedback")
        return v if v is not None else 0.0
