"""
管式反应器 — 标准设备类模板 (Device Class Template)

定义「管式反应器」这一设备大类的标准动作(action)与状态属性(property)。
...
@device(
    ...
    description="管式反应器标准接口：同一大类跨品牌统一的动作与参数。",
    displayname="管式反应器",
)
class TubularReactor:
    ...
    def __init__(self, device_id: Optional[str] = None, config: Optional[Dict[str, Any]] = None, **kwargs):
        ...
        Args:
            ...
            config[设备配置]: 设备启动配置。
        """
        self.device_id = device_id or "tubular_reactor"
    ...
    def current_backpressure(self) -> int:
        """实际背压监测。"""
        return self.data.get("current_backpressure", 0)
