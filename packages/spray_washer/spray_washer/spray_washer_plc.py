"""
喷淋式清洗机 — PLC(OPC UA) 交互驱动 (自动生成，请勿手改)

由 generate_plc_drivers.py 依据 device_templates_actions.csv 生成。
继承 OpcUaClientWithSubscription，把「喷淋式清洗机」标准动作/属性映射为 PLC 节点读写：
...
@device(
    id="spray_washer_plc",
    category=["喷淋式清洗机"],
    description="喷淋式清洗机 PLC(OPC UA) 交互驱动：动作映射为节点读写。",
    display_name="喷淋式清洗机(PLC)",
)
class SprayWasherPLC(OpcUaClientWithSubscription):
    ...
    def set_spray_pressure(self, spray_pressure: float = 0.0) -> Dict[str, Any]:
        """设置喷淋压力：写入设定值节点 Spray_Pressure_Setpoint。"""
        self.set_node_value("Spray_Pressure_Setpoint", spray_pressure)
        return {"success": True, "message": "设置喷淋压力已下发", "spray_pressure": spray_pressure}

    @action(description="设置喷淋温度")
    def set_spray_temperature(self, spray_temperature: float = 0.0) -> Dict[str, Any]:
        """设置喷淋温度：写入设定值节点 Spray_Temperature_Setpoint。"""
        self.set_node_value("Spray_Temperature_Setpoint", spray_temperature)
        return {"success": True, "message": "设置喷淋温度已下发", "spray_temperature": spray_temperature}

    @action(description="设置喷淋时间")
    def set_spray_time(self, spray_time: float = 0.0) -> Dict[str, Any]:
        """设置喷淋时间：写入设定值节点 Spray_Time_Setpoint。"""
        self.set_node_value("Spray_Time_Setpoint", spray_time)
        return {"success": True, "message": "设置喷淋时间已下发", "spray_time": spray_time}

    @action(description="设置洗涤剂浓度")
    def set_detergent_concentration(self, detergent_concentration: float = 0.0) -> Dict[str, Any]:
        """设置洗涤剂浓度：写入设定值节点 Detergent_Concentration_Setpoint。"""
        self.set_node_value("Detergent_Concentration_Setpoint", detergent_concentration)
        return {"success": True, "message": "设置洗涤剂浓度已下发", "detergent_concentration": detergent_concentration}
    ...
    def spray_pressure_feedback(self) -> float:
        """喷淋压力反馈（读节点 Spray_Pressure_Feedback）。"""
        v = self.get_node_value("Spray_Pressure_Feedback")
        return v if v is not None else 0.0

    @property
    @topic_config()
    def spray_temperature_feedback(self) -> float:
        """喷淋温度反馈（读节点 Spray_Temperature_Feedback）。"""
        v = self.get_node_value("Spray_Temperature_Feedback")
        return v if v is not None else 0.0

    @property
    @topic_config()
    def spray_time_feedback(self) -> float:
        """喷淋时间反馈（读节点 Spray_Time_Feedback）。"""
        v = self.get_node_value("Spray_Time_Feedback")
        return v if v is not None else 0.0

    @property
    @topic_config()
    def detergent_concentration_feedback(self) -> float:
        """洗涤剂浓度反馈（读节点 Detergent_Concentration_Feedback）。"""
        v = self.get_node_value("Detergent_Concentration_Feedback")
        return v if v is not None else 0.0
