"""
蒸馏水设备 — PLC(OPC UA) 交互驱动 (自动生成，请勿手改)

由 generate_plc_drivers.py 依据 device_templates_actions.csv 生成。
继承 OpcUaClientWithSubscription，把「蒸馏水设备」标准动作/属性映射为 PLC 节点读写：
...
@device(
    id="distilled_water_system_plc",
    category=["蒸馏水设备"],
    description="蒸馏水设备 PLC(OPC UA) 交互驱动：动作映射为节点读写。",
    display_name="蒸馏水设备(PLC)",
)
class DistilledWaterSystemPLC(OpcUaClientWithSubscription):
    ...
    def set_distillation_flow(self, distillation_flow: float = 0.0) -> Dict[str, Any]:
        """设置蒸馏流量：写入设定值节点 Distillation_Flow_Setpoint。"""
        self.set_node_value("Distillation_Flow_Setpoint", distillation_flow)
        return {"success": True, "message": "设置蒸馏流量已下发", "distillation_flow": distillation_flow}

    @action(description="蒸馏")
    def distill(self) -> Dict[str, Any]:
        """蒸馏：写 Distill_Trigger 触发，等待 Distill_Complete 完成后复位。"""
        logger.info("蒸馏...")
        self.set_node_value("Distill_Trigger", True)
        if not self._wait_until_true("Distill_Complete", description="蒸馏完成"):
            raise ValueError("蒸馏失败：动作未完成")
        self.set_node_value("Distill_Trigger", False)
        if not self._wait_until_false("Distill_Complete", description="蒸馏完成复位"):
            raise ValueError("蒸馏失败：完成状态复位超时")
        return {"success": True, "message": "蒸馏完成"}
    ...
    def current_distillation_flow(self) -> float:
        """实际蒸馏流量（读节点 Current_Distillation_Flow）。"""
        v = self.get_node_value("Current_Distillation_Flow")
        return v if v is not None else 0.0

    @property
    @topic_config()
    def condensate_temperature(self) -> float:
        """冷凝水温度（读节点 Condensate_Temperature）。"""
        v = self.get_node_value("Condensate_Temperature")
        return v if v is not None else 0.0
