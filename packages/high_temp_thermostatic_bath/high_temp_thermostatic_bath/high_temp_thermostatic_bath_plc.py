"""
高温恒温槽 — PLC(OPC UA) 交互驱动 (自动生成，请勿手改)

由 generate_plc_drivers.py 依据 device_templates_actions.csv 生成。
继承 OpcUaClientWithSubscription，把「高温恒温槽」标准动作/属性映射为 PLC 节点读写：
...
class HighTempThermostaticBathPLC(OpcUaClientWithSubscription):
    ...
    @action(description="设置报警灵敏度")
    def set_alarm_sensitivity(self, alarm_sensitivity: int = 0) -> Dict[str, Any]:
        """设置报警灵敏度：写入设定值节点 Alarm_Sensitivity_Setpoint。"""
        self.set_node_value("Alarm_Sensitivity_Setpoint", alarm_sensitivity)
        return {"success": True, "message": "设置报警灵敏度已下发", "alarm_sensitivity": alarm_sensitivity}
