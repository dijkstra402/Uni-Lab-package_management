...
定义「深低温冰箱」这一设备**大类**的标准动作(`@action`)与状态属性(`@property`)。
...
```
deep_low_temp_freezer/
├── pyproject.toml
├── requirements.txt
├── .github/workflows/check_registry.yml
└── deep_low_temp_freezer/
    ├── __init__.py
    └── deep_low_temp_freezer.py        # 深低温冰箱 标准类定义
```

## 本地验证

```bash
unilab --check_mode --devices ./deep_low_temp_freezer --external_devices_only
```
